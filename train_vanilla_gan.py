...
class VanillaGAN:
    def __init__(self, BATCH_SIZE = 32, loss_function = 'minimax_loss'):
        ...
        self.mnist_dataset = load_dataset()
    ...
    def __train_step(self, train_image):
        random_tensor = sample_gaussian_noise(train_image.shape[0], self.random_noise)
        with tf.GradientTape() as gen_tape, tf.GradientTape() as dis_tape:
            pred_image = self.generator_model(random_tensor, training=True)
    ...
    def train(self, EPOCHS = 50):
        ...
        for epoch in range(EPOCHS):
            ...
            for train_image_batch in train_dataset:
                generator_loss, discriminator_loss = self.__train_step(train_image_batch)
